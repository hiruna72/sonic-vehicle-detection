...
class ExampleApp(QtGui.QMainWindow, ui_main.Ui_MainWindow):
    def __init__(self, parent=None):
        pyqtgraph.setConfigOption('background', 'w') #before loading widge
    ...
    def update(self):
        if not self.ear.data is None and not self.ear.fft is None:
            ...
            isDetected = False
            for x in self.ear.fft/self.maxFFT:
                if (x >= 0.5):
                	isDetected = True
                	break
            ...
            if isDetected:
                ...
                self.label_3.setText("Vehicle detected!")
            else:
                self.label_3.setStyleSheet('color: green')
                self.label_3.setText("No Vehicles detected!")
